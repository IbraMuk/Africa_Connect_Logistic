-- Script de création de la table marchandises
-- Exécuter ce script dans PostgreSQL pour créer la table
...
CREATE TABLE IF NOT EXISTS marchandises (
    ...
    categorie VARCHAR(20) NOT NULL DEFAULT 'general' 
        CHECK (categorie IN ('general', 'dangerux', 'perissable', 'electronique', 'vehicule', 'textile', 'agricole', 'standard', 'fragile', 'dangereux', 'lourde', 'volumineux')),
...
CREATE INDEX idx_marchandise_categorie ON marchandises(categorie);
...
INSERT INTO marchandises (designation, poids, volume, expediteurId, destinataireNom, destinataireTelephone, 
                          villeDepart, villeArrivee, dateEnvoi, dateLivraisonPrevue, statut, typeTransport, coutTransport)
VALUES 
    ('Équipements électroniques', 150.50, 0.5, 1, 'Société ABC', '+243812345678', 
     'Kinshasa', 'Lubumbashi', CURRENT_DATE, CURRENT_DATE + INTERVAL '7 days', 'En attente', 'Routier', 250.00),
    ('Documents importants', 5.20, 0.02, 2, 'M. Jean Dupont', '+243823456789', 
     'Kinshasa', 'Bukavu', CURRENT_DATE - INTERVAL '2 days', CURRENT_DATE + INTERVAL '3 days', 'En transit', 'Aérien', 180.00),
    ('Produits pharmaceutiques', 200.00, 0.8, 3, 'Pharma Congo', '+243834567890', 
     'Matadi', 'Kisangani', CURRENT_DATE - INTERVAL '5 days', CURRENT_DATE - INTERVAL '1 day', 'Livré', 'Routier', 320.00);

-- Affichage de confirmation
SELECT 'Table marchandises créée avec succès!' AS message;
